...
export function resolveReplyLanguage(text: string, previous: ReplyLanguage = 'hinglish', autoDetect = true): ReplyLanguage {
  const prose = text
    .replace(/```[\s\S]*?(?:```|$)/g, ' ')
    .replace(/`[^`]*`/g, ' ')
    .replace(/^\s*>.*$/gm, ' ')
  ...
  // When the user has already committed to a language (via the /start picker),
  // keep it locked — only a direct request above may change it.
  if (!autoDetect) return previous;
